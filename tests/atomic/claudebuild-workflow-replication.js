...
class EnhancedAgentRuntime {
  ...
  async generateFiles(task) {
    ...
    files.set(`${task.name || 'component'}.js`, this.generateMainFile(task));
    
    // Generate test file
    files.set(`${task.name || 'component'}.test.js`, this.generateTestFile(task));
  ...
  generateMainFile(task) {
    const name = task.name || 'Component';
    return `/**
 * ${name} - Generated by ClaudeBuild v2
 * ${task.description}
 */
...
module.exports = ${this.capitalize(name)};
`;
  }
...
const ${this.capitalize(name)} = require('./${name}');

describe('${this.capitalize(name)}', () => {
  let instance;

  beforeEach(() => {
    instance = new ${this.capitalize(name)}();
  });
  ...
  test('should throw error if not initialized', async () => {
    await expect(instance.execute()).rejects.toThrow('${name} not initialized');
  });
});
`;
  }
...
${task.description || 'Generated by ClaudeBuild v2'}
...
const instance = new ${this.capitalize(name)}();
await instance.initialize();
const result = await instance.execute();
\`\`\`
...
### \`initialize()\`
Initializes the ${name} component.
...
Generated by ClaudeBuild v2 - Multi-agent AI development orchestrator
`;
  }
...
class SessionManager {
  ...
  async createWorktree(session) {
    try {
      ...
      try {
        ...
        this.logger.info(`Branch feature/${session.id} already exists, using existing branch`);
        await execAsync(`git checkout feature/${session.id}`, { cwd: this.basePath });
      }
      
      // Try to create worktree, but handle if it already exists
      try {
        ...
        this.logger.warn(`Worktree already exists or failed to create: ${worktreeError.message}`);
        // Fallback to regular directory
        await fs.mkdir(session.worktree, { recursive: true });
      }
    } catch (error) {
      this.logger.warn(`Failed to setup git operations: ${error.message}`);
      // Fallback to regular directory without git
      await fs.mkdir(session.worktree, { recursive: true });
    }
  }
  ...
  async checkpoint(sessionId, message) {
    ...
    try {
      ...
      await execAsync(`git add -A`, { cwd: session.worktree });
      await execAsync(`git commit -m "${message}" --allow-empty`, { cwd: session.worktree });
      ...
      // Still record the checkpoint without git commit
      session.checkpoints.push({
        timestamp: new Date(),
        message,
        commit: null
